...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Persist message data nodes inside your database registry
    ...
    // 2. Safe execution layer: Send admin notification via Brevo HTTP API
            ...
            htmlContent: `
              <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 2rem; background: #FAF7F4;">
                <div style="background: #1a1a1a; padding: 2rem; text-align: center; margin-bottom: 2rem; border-bottom: 2px solid #C9A84C;">
                  <h1 style="color: white; margin: 0; font-size: 1.3rem; letter-spacing: 0.1em; uppercase">New Message Received</h1>
                </div>
    ...
    return NextResponse.json(
      { error: "Internal server exception drop" },
      { status: 500 }
    );
  }
}
